...
class Manager extends Component {
  ...
  onChangeTask = (task, id) => {
    ...
    arrayCol.map(col => {
      if (col.id == id) col.tasks.push(task);
    });
  };
  ...
  handleChange = e => {
    ...
    let project = this.props.projectList.filter(
      prj => prj.id === e.target.value,
    );
    ...
    return (
      <Fragment>
        ...
        <select
          ...
          value={this.props.projectSelected.id}
        >
        ...
        <UnlockAchievement
          ...
          description="Parabéns pela primeira compra em nossa loja."
          points="1.500 Pt"
          closeAchievement={() => this.setState({ unlock: false })}
          Achievement={this.state.unlock}
        />
        <div className="columnContainer">
          {this.props.projectSelected.boards &&
            this.props.projectSelected.boards.map(col => {
              return (
                <Column
                  id={col.id}
                  onChange={this.props.changeColumn}
                  name={col.name}
                  canDragBool={this.props.canDragBool}
                  tasks={col.tasks}
                />
              );
            })}
